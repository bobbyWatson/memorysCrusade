		...
		if(this.hitBox.GetBody().GetType()==1){
			...
			if(this.joint !== undefined){
				if(this.joint.m_bodyB.GetUserData().tag!== "Player"){
					this.joint.m_bodyB.ApplyImpulse(new Box2D.Vec2(moveVec.x * 3,moveVec.y) ,this.joint.m_bodyB.GetWorldCenter());
				}else{
					this.joint.m_bodyA.ApplyImpulse(new Box2D.Vec2(moveVec.x * 3,moveVec.y),this.joint.m_bodyA.GetWorldCenter());
				}
			}
